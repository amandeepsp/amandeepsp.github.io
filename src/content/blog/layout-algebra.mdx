...
and the two modes merge into $s_0 \times s_1 : d_0$. We can also see that $1$ shape is an identity in reduction. i.e. $1: d'$ and $s: d$ reduce to $s: d$. This is useful when we need to simplify some particularly gnarly tensor layouts.
...
<Figure
  ...
  width="100%"
>
  <Fragment slot="caption">Visualization of complement of a layout</Fragment>
</Figure>
...
First we need the complement of $A$ with respect to $M = \text{size}(A) \times \text{size}(B) = 12$. We get $A^*_{12} = 3:4$ — the three offsets $\{0, 4, 8\}$ where copies begin.
