---
layout: post
title:  "Functional Programming is Awesome"
description: "Journey to the Ivory tower :relieved:"
date:   2020-08-12 10:55:06 +0530
categories: [Functional Programming]
excerpt_separator: <!--more-->
permalink: /fp-awesome/
---
...
*Side-effects* is just jargon for the fact that a function with the same inputs produces same results. This fact breaks down if a function manipulates a mutable variable, we can't ensure the "no side-effects" rule, because the mutable variable's value could have changed between subsequent function calls.
...
<br/>
I would argue that these concepts are still worth learning as it disciplines you to think a program from a different perspective than that of imperative or OOP paradigm and makes you write programs which are not necessarily functional but contains elements of FP.
...
Functional languages try to provide two kinds of glues Higher-order functions and Lazy evaluation. Higher-order functions are nowadays present in most modern programming languages. Lazy evaluation is a kind of controversial topics, some functional languages have it, some choose not to and some implement it partially (e.g. Scheme providing only lazy lists).
...
We can see a kind of abstraction emerging here, the operator and the starting value can be. This is not possible in languages that do have higher-order functions. Here `foldr` accumulates values form right.
