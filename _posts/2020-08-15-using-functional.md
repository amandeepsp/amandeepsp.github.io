...
description: "A Journey to the Ivory tower :european_castle:"
...
2. Functions evaluation could not have any side-effects. You might ask what side-effects? :thinking: functions.. side-effects? *Side-effects* is just jargon for the fact that your functions should not make any changes to the things that outside of its scope, e.g. changing a mutable state, performing I/O and calling other functions that have side effects.

These facts combined make your functions more composable, enhancing your ability to compose large systems out of very simple components. 
:warning: **The code here is written in [Haskell][haskell], but the concepts are language agnostic. Different languages choose and implement these concepts differently.**
...
The benefits might not seem obvious at first and are hugely language dependent but knowing these concepts is also a win. Other times functional programming's inspirations from [Category Theory][cattheo] is a big turnoff for developers. This is aptly summed up in this video adapted from the movie *Downfall*.
...
FP provides *referential transparency* to programs, means that an *expression* can be replaced by its corresponding *value*, and this operation doesn't change the execution of the program. To achieve this your expressions must be *pure*, you cannot guarantee this in presence of a mutable state as the value in now dependent on a state. 
